...
def nueva():
    if request.method == "POST":
        ...
        if postulante_id:
            try:
                postulante_id = int(postulante_id)
            except Exception:
                postulante_id = None
        ...
        if not postulante_id:
            ...
            if dni_val:
                ...
                cur_p.execute(
                    "SELECT id FROM postulantes WHERE dni=%s LIMIT 1", (dni_val,)
                )
                row_p = cur_p.fetchone()
                if row_p:
                    postulante_id = row_p["id"]
                cur_p.close()
                conn_p.close()

        if postulante_id:
            conn_dup = get_connection()
            cur_dup = conn_dup.cursor(dictionary=True)
            cur_dup.execute(
                "SELECT id, codigo FROM solicitudes WHERE postulante_id=%s LIMIT 1",
                (postulante_id,),
            )
            ...
            if dup:
                flash(
                    f"Este postulante ya tiene la solicitud {dup['codigo']} registrada.",
                    "warning",
                )
        ...
        if carrera_id:
            try:
                ...
                if carrera:
                    ...
                    session["costo_examen_carrera"] = float(
                        carrera["costo_examen"] or 130
                    )
                cur_c.close()
                conn_c.close()
            except Exception:
        ...
        costo_exam = session.get(
            "costo_examen_carrera", float(data.get("costo_examen", 130))
        )
...
def editar(id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            """,
                (
                    data.get("fecha_emision") or now_pe().strftime("%Y-%m-%d"),
                    data.get("observacion", ""),
                    float(data.get("costo_credito", 60)),
                    float(data.get("costo_examen", 130)),
                    id,
                ),
            )
            conn.commit()
        ...
        for item in data.get("cursos", []):
            ...
            """,
                (
                    id,
                    item["curso_local_id"],
                    item.get("curso_externo_id"),
                    item.get("nota"),
                    item.get("estado", "pendiente"),
                    item.get("periodo_lectivo"),
                    usuario_id,
                ),
            )
...
def eliminar_curso(curso_id):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT solicitud_id FROM solicitud_cursos WHERE id=%s", (curso_id,)
        )
        row = cur.fetchone()
...
def marcar_emitido(id):
    ...
    try:
        ...
        registrar(
            "emitir",
            "solicitudes",
            f"Solicitud emitida por {usuario_nombre}: id={id}",
            id,
        )
...
def enviar_correo(id):
    ...
    try:
        ...
        if sol["estado"] != "emitido":
            return jsonify(
                {"ok": False, "error": "Solo se pueden enviar solicitudes emitidas"}
            )
        ...
        with tempfile.NamedTemporaryFile(
            suffix=ruta_adjunto_suffix, delete=False
        ) as tmp:
        ...
        if template_id:
            cur.execute(
                "SELECT cuerpo FROM plantillas_correo WHERE id=%s AND activo",
                (template_id,),
            )
            row = cur.fetchone()
            cuerpo_base = row["cuerpo"] if row else ""
        else:
            cuerpo_base = html.escape(mensaje)

        def render_html(template, **vars):
            result = template
            for k, v in vars.items():
                safe = str(html.escape(str(v)))
                result = result.replace("@" + k, safe)
                result = result.replace("{{" + k + "}}", safe)
            return result
        ...
        if resultado["ok"]:
            registrar(
                "enviar_correo",
                "solicitudes",
                f"Correo enviado a {destinatario} para solicitud {sol['codigo']} con confirmacion",
                id,
            )
            return jsonify({"ok": True})
        else:
            return jsonify(
                {"ok": False, "error": resultado.get("error", "Error desconocido")}
            )
...
def correo_preview(id):
    ...
    from routes.correos import get_plantillas
    from routes.generar_word import generar_pdf
    ...
    try:
        ...
        fecha = (
            sol["fecha_emision"].strftime("%d/%m/%Y")
            if sol.get("fecha_emision")
            else now_pe().strftime("%d/%m/%Y")
        )
...
def whatsapp_preview(id):
    ...
    try:
        ...
        correo_mensaje = (
            f"y enviada a su correo {sol['correo_postulant']}"
            if sol["correo_postulant"]
            else ""
        )
        _ = correo_mensaje  # reserved
    ...
    try:
        ...
        if sol:
            registrar(
                "enviar_whatsapp",
                "solicitudes",
                f"WhatsApp enviado para solicitud {sol['codigo']}",
                id,
            )
        return jsonify({"ok": True})
    except Exception:
...
def confirmar_solicitud(token):
    ...
    try:
        ...
        registrar(
            "confirmacion",
            "solicitudes",
            f"Postulante confirmo la solicitud {sol['codigo']}",
            sol["id"],
        )
...
def rechazar_solicitud(token):
    ...
    if request.method == "POST":
        ...
        registrar(
            "rechazo",
            "solicitudes",
            f"Postulante rechazo la solicitud {sol['codigo']}. Motivo: {motivo}",
            sol["id"],
        )
...
def consolidado_excel(id):
    ...
    try:
        ...
        ws.cell(
            row=row, column=2, value=sol.get("carrera_nombre", "")
        ).font = subtitle_font
        row += 2
        ...
        ws.cell(row=row, column=1, value="ASIGNATURAS DEL PLAN EXTERNO").font = Font(
        ...
        ws.cell(row=row, column=6, value="CONVALIDACIÓN").font = Font(
            bold=True, size=10, color="1F3864"
        )
        ...
        for idx, c in enumerate(cursos):
            ...
            ws.cell(
                row=row, column=3, value=c.get("nombre_curso", "")
            ).font = normal_font
            ws.cell(row=row, column=3).alignment = left_align
            ws.cell(
                row=row, column=4, value=cred_local if cred_local else "-"
            ).font = normal_font
            ws.cell(row=row, column=4).alignment = center_align
            ws.cell(
                row=row, column=5, value=c.get("prerrequisito", "")
            ).font = normal_font
            ws.cell(row=row, column=5).alignment = center_align

            ws.cell(
                row=row, column=6, value=c.get("convalidacion", "")
            ).font = normal_font
            ws.cell(row=row, column=6).alignment = left_align
            cred_ext = c.get("cred", 0) or 0
            ws.cell(
                row=row, column=7, value=cred_ext if cred_ext else "-"
            ).font = normal_font
            ...
            if estado == "convalidado":
                ...
                nota_str = (
                    ...
                    else (str(nota_val) if nota_val is not None else "-")
                )
            else:
                nota_str = "-"
...
def consolidado_preview(id):
    ...
    try:
        ...
        if not sol:
            return (
                '<div style="padding:2rem;text-align:center;color:#dc2626;">Solicitud no encontrada</div>',
                404,
            )
        ...
        for c in cursos:
            ...
            if estado == "convalidado":
                ...
                if nota_val is not None:
                    ...
                    nota_html = (
                        f'<span style="color:#16a34a;font-weight:bold;">{nota_display}</span>'
                        if nota_val >= 11
                        else f'<span style="color:#dc2626;font-weight:bold;">{nota_display}</span>'
                    )
                else:
                    nota_html = "-"
            else:
...
def historial_solicitud(id):
    ...
    try:
        ...
        for log in logs:
            log["fecha"] = (
                log["fecha"].strftime("%d/%m/%Y %H:%M") if log["fecha"] else None
            )
...
def record_notas(id):
    ...
    try:
        ...
        if ultimo_registro and ultimo_registro.get("nombre_completo"):
            fecha_ult = (
                ultimo_registro["fecha_registro"].strftime("%d/%m/%Y %H:%M")
                if ultimo_registro.get("fecha_registro")
                else ""
            )
            ...
            if ciclo in cursos_por_ciclo:
                for c in cursos_por_ciclo[ciclo]:
                    ...
                    if nota_val is not None:
                        ...
                        nota_html = (
                            f'<span class="nota-aprobado">{nota_display}</span>'
                            if nota_val >= 11
                            else f'<span class="nota-desaprobado">{nota_display}</span>'
                        )
                    else:
                        nota_html = "-"
...
def record_notas_pdf(id):
    ...
    try:
        ...
        if ultimo_registro and ultimo_registro.get("nombre_completo"):
            ...
            ultimo_notas_pdf = f" - Ultimo registro de notas por: {html.escape(ultimo_registro['nombre_completo'])} {html.escape(fecha_ult)}"
        ...
        subtitle_style = ParagraphStyle(
            ...
            fontSize=11,
            alignment=1,
            textColor=colors.HexColor("#4a7cc7"),
        )
        normal_style = styles["Normal"]

        elements.append(Paragraph("RECORD DE NOTAS", title_style))
        elements.append(
            Paragraph(sol.get("institucion") or "Institución de Origen", subtitle_style)
        )
        ...
        info_data = [
            ...
            [
                "Plan Estudios:",
                sol.get("plan_nombre") or "-",
                "Periodo:",
                sol.get("periodo_academico") or "-",
            ],
        ]
        info_table = Table(
            info_data, colWidths=[1.2 * inch, 2.3 * inch, 1 * inch, 2 * inch]
        )
        info_table.setStyle(
            TableStyle(
                [
        ...
        for c in cursos:
            ...
            table_data.append(
                [
                    ...
                    c.get("ciclo", ""),
                    c.get("curso_codigo", ""),
                    Paragraph(c.get("nombre_curso", "")[:45], normal_style),
                    str(c.get("creditos", 0)),
                    nota_str,
                ]
            )
            nro += 1
        ...
        elements.append(Spacer(1, 0.2 * inch))
        elements.append(
            Paragraph(f"<b>Total Créditos:</b> {total_creditos}", normal_style)
        )
        elements.append(Spacer(1, 0.3 * inch))
        elements.append(
            Paragraph(
                f"Documento generado automáticamente - Sistema de Convalidaciones UAI - {now_pe().strftime('%d/%m/%Y %H:%M')}{ultimo_notas_pdf}",
                ParagraphStyle(
                    "Footer", fontSize=8, alignment=1, textColor=colors.grey
                ),
            )
        )
