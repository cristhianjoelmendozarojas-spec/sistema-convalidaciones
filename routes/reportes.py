...
def descargar_uno():
    ...
    try:
        ...
        if row:
            ...
            prog = (
                (row["programa"] or "SinPrograma")[:30]
                .replace(" ", "_")
                .replace("/", "-")
            )
        ...
        with zipfile.ZipFile(buffer_zip, "w", zipfile.ZIP_DEFLATED) as zf:
            for item in solicitudes:
                if isinstance(item, dict):
                    ...
                    prog = (
                        (item.get("programa", "") or "SinProg")[:20]
                        .replace(" ", "_")
                        .replace("/", "-")
                    )
                else:
            ...
            if descargados == 0:
                return jsonify(
                    {"ok": False, "error": "No se pudieron generar PDFs"}
                ), 400
    ...
    try:
        ...
        with zipfile.ZipFile(buffer_zip, "w", zipfile.ZIP_DEFLATED) as zf:
            for grupo_nombre, solicitudes in grupos_data.items():
                ...
                sol_ids = [
                    s["solicitud_id"] for s in solicitudes if s.get("solicitud_id")
                ]
                cant = len(sol_ids)
