...
def _construir_mensaje(remitente_nombre, correo_remitente, destinatario, asunto, cuerpo_html, adjuntos=None):
...
def _enviar_brevo(msg, correo_remitente, destinatario):
    ...
    try:
        ...
        return {'ok': False, 'error': 'Error de autenticación con Brevo. Verifica BREVO_SMTP_PASSWORD en Render.'}
    except smtplib.SMTPConnectError:
        return {'ok': False, 'error': f'No se pudo conectar a {BREVO_HOST}:{BREVO_PUERTO}.'}
    except TimeoutError:
...
def _enviar_smtp_directo(msg, correo_remitente, destinatario, config):
    ...
    try:
        ...
        return {'ok': False, 'error': 'Error de autenticación. Para Gmail usa contraseña de aplicación (16 caracteres).'}
    except smtplib.SMTPConnectError:
        return {'ok': False, 'error': f'No se pudo conectar a {smtp_host}:{smtp_puerto}.'}
    except TimeoutError:
        return {'ok': False, 'error': 'Tiempo de espera agotado conectando al servidor SMTP.'}
    except Exception as e:
...
def enviar_correo(destinatario, asunto, cuerpo_html, adjuntos=None, usuario_id=None):
    ...
    msg = _construir_mensaje(nombre_remitente, correo_remitente, destinatario, asunto, cuerpo_html, adjuntos)
...
def get_estado_correo():
    ...
    if BREVO_PASSWORD:
        ...
        correo = config.get('correo_remitente', 'no configurado') if config else 'no configurado'
        return {
            'configurado': True,
