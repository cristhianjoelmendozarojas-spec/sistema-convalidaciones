...
CAMPO_LABELS = {
    ...
    "turno": "Turno",
    "asesora": "Asesora",
    "fecha_registro_origen": "Fecha registro",
    "escala_matricula": "Escala matrícula",
    "escala_pensiones": "Escala pensiones",
    "monto_expediente": "Monto expediente",
    "estado_expediente": "Estado expediente",
    "fecha_pago_expediente": "Fecha pago expediente",
    "monto_postulacion": "Monto postulación",
    "estado_postulacion": "Estado postulación",
    "fecha_pago_postulacion": "Fecha pago postulación",
...
def _upsert_postulante(cur, fila: dict, fila_raw: tuple = None):
    ...
    try:
        edad = (
            int(str(edad).strip())
            if edad and str(edad).strip() not in ("", "nan", "None")
            else None
        )
    except Exception:
        edad = None
    ...
    valores = (
        ...
        fila.get("distrito", ""),
        fila.get("sexo", ""),
        _limpiar_fecha(fila.get("fecha_nacimiento")),
        edad,
        fila.get("local", ""),
        fila.get("facultad", ""),
        str(fila.get("programa", "")).strip().upper(),
        fila.get("modalidad_admision", ""),
        fila.get("semestre_academico", ""),
        fila.get("modalidad_estudios", ""),
        fila.get("turno", ""),
        fila.get("asesora", ""),
        _limpiar_fecha(fila.get("fecha_registro_origen")),
        fila.get("escala_matricula", ""),
        fila.get("escala_pensiones", ""),
        _limpiar_monto(fila.get("monto_expediente")),
        _estado(IDX_ESTADO_EXPEDIENTE),
        _limpiar_fecha(fila.get("fecha_pago_expediente")),
        _limpiar_monto(fila.get("monto_postulacion")),
        _estado(IDX_ESTADO_POSTULACION),
        _limpiar_fecha(fila.get("fecha_pago_postulacion")),
        _limpiar_monto(fila.get("monto_matricula")),
        _estado(IDX_ESTADO_MATRICULA),
...
def index():
    ...
    if programa:
        where.append("programa=%s")
        params.append(programa)
    if modalidad:
        where.append("modalidad_estudios=%s")
        params.append(modalidad)

    cur.execute(
        f"SELECT COUNT(*) as total FROM postulantes p WHERE {' AND '.join(where)}",
        params,
    )
    ...
    if estado:
        if estado == "emitido":
            postulantes = [
                p for p in postulantes if p.get("solicitud_estado") == "emitido"
            ]
        elif estado == "en_proceso":
            postulantes = [
                p
                for p in postulantes
                if p.get("solicitud_estado") in ("borrador", None)
            ]
        elif estado == "sin_solicitud":
            postulantes = [p for p in postulantes if not p.get("solicitud_id")]

    cur.execute(
        "SELECT DISTINCT programa FROM postulantes WHERE programa IS NOT NULL ORDER BY programa"
    )
    programas = [r["programa"] for r in cur.fetchall()]
    cur.execute(
        "SELECT DISTINCT modalidad_estudios FROM postulantes WHERE modalidad_estudios IS NOT NULL ORDER BY modalidad_estudios"
    )
    ...
    return render_template(
        "postulantes/lista.html",
        postulantes=postulantes,
        programas=programas,
        modalidades=modalidades,
        q=q,
        programa_sel=programa,
        modalidad_sel=modalidad,
        estado_sel=estado,
...
def importar():
    if request.method == "GET":
        ...
        return render_template(
            "postulantes/importar.html", pendientes_session=pendientes
        )
    ...
    try:
        ...
        for nombre_col, campo_bd in COLUMNAS.items():
            ...
            try:
                ...
                try:
                    col_map[campo_bd] = [h.casefold() for h in enc_nfc].index(
                        nfc_col.casefold()
                    )
                except ValueError:
                    pass
        ...
        if "codigo" not in col_map:
            ...
            flash(
                f'El archivo no tiene la columna "Código de estudiante". Columnas encontradas: {encontradas}',
                "danger",
            )
            return redirect(url_for("postulantes.importar"))
...
_CAMPOS_FECHA = {
    "fecha_nacimiento",
    "fecha_registro_origen",
    "fecha_pago_expediente",
    "fecha_pago_postulacion",
...
def eliminar(id):
    ...
    try:
        cur.execute(
            "SELECT id, codigo, estado FROM solicitudes WHERE postulante_id=%s", (id,)
        )
        ...
        if force:
            ...
            registrar(
                "eliminar",
                "postulantes",
                f"Postulante eliminado (FORZADO) con {len(solicitudes)} solicitud(es): {codigos}",
                id,
            )
...
def eliminar_masivo():
    ...
    try:
        ...
        cur.execute(
            f"SELECT id, codigo, estado FROM solicitudes WHERE postulante_id IN ({placeholders})",
            ids,
        )
        solicitudes = cur.fetchall()
        tiene_emitidas = any(s["estado"] == "emitido" for s in solicitudes)
        ...
        if tiene_emitidas:
            return jsonify(
                {
                    "ok": False,
                    "error": "No se puede eliminar: hay postulantes con solicitudes EMITIDAS. Elimínalos uno por uno con fuerza.",
                }
            ), 400
        ...
        cur.execute(
            f"DELETE FROM checklist_documentos WHERE postulante_id IN ({placeholders})",
            ids,
        )
        cur.execute(f"DELETE FROM postulantes WHERE id IN ({placeholders})", ids)
        conn.commit()
        registrar(
            "eliminar", "postulantes", f"Eliminación masiva: {len(ids)} postulante(s)"
        )
...
def ver(id):
    ...
    cur.execute(
        "SELECT * FROM checklist_documentos WHERE postulante_id=%s ORDER BY id", (id,)
    )
    items = cur.fetchall()
    total = len(items)
    ...
    conn.close()
    return render_template(
        "postulantes/detalle.html", p=p, items=items, total=total, entregados=entregados
    )
...
def _asegurar_checklist_docs(cur, postulante):
    ...
    cur.execute(
        "SELECT documento FROM checklist_documentos WHERE postulante_id=%s",
        (postulante["id"],),
    )
...
def checklist(id):
    ...
    cur.execute(
        "SELECT id, apellidos_nombres, programa FROM postulantes WHERE id=%s", (id,)
    )
    p = cur.fetchone()
    if not p:
        flash("Postulante no encontrado.", "danger")
    ...
    cur.execute(
        "SELECT documento FROM checklist_documentos WHERE postulante_id=%s", (id,)
    )
    existentes = {r["documento"] for r in cur.fetchall()}
...
def guardar_item(item_id):
    ...
    try:
        if "archivo" in d:
            cur.execute(
                "UPDATE checklist_documentos SET archivo=NULL WHERE id=%s", (item_id,)
            )
            conn.commit()
...
def upload_item_file(item_id):
    ...
    try:
        ...
        cur = conn.cursor(dictionary=True)
        cur.execute(
            "SELECT postulante_id FROM checklist_documentos WHERE id=%s", (item_id,)
        )
        row = cur.fetchone()
        if not row:
            cur.close()
...
def editar(id):
    ...
    try:
        ...
        registrar(
            "editar", "postulantes", f"Datos actualizados para postulado id={id}", id
        )
        return jsonify({"ok": True})
    except Exception as e:
        conn.rollback()
...
def actualizar_pago(id):
    ...
    try:
        ...
        registrar(
            "editar",
            "postulantes",
            f"Pago {tipo} actualizado para postulante id={id}",
            id,
        )
        return jsonify({"ok": True})
    except Exception as e:
...
def crear():
    ...
    try:
        ...
        if cur.fetchone():
            return jsonify(
                {"ok": False, "error": f"El DNI {dni} ya está registrado"}
            ), 400
