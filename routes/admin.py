...
def editar_usuario(uid):
    ...
    try:
        cur.execute("""
            ...
            WHERE id=%s
        """, (d.get('nombre_completo','').strip(), d.get('rol','usuario'),
              d.get('estado','activo'), uid))
        conn.commit()
...
def crear_facultad():
    ...
    try:
        cur.execute("""
            ...
            VALUES (%s, %s, %s, %s)
        """, (d['nombre'].strip(), d.get('codigo','').strip(),
              d.get('descripcion','').strip(), d.get('estado','activo')))
        conn.commit()
...
def editar_facultad(fid):
    ...
    try:
        cur.execute("""
            ...
            WHERE id=%s
        """, (d['nombre'].strip(), d.get('codigo','').strip(),
              d.get('descripcion','').strip(), d.get('estado','activo'), fid))
        conn.commit()
        return jsonify({'ok': True})
    except Exception as e:
        conn.rollback(); return jsonify({'ok': False, 'error': str(e)}), 400
...
def carreras():
    ...
    facultades = cur.fetchall()
    cur.close(); conn.close()
    return render_template('admin/carreras.html', carreras=carreras, facultades=facultades)
...
def guardar_modulos_usuario(uid):
    ...
    try:
        ...
        if claves:
            ...
            """.format(','.join(['%s'] * len(claves))), [uid] + claves)

        conn.commit()
        registrar('editar', 'usuarios', f'Módulos actualizados para usuario id={uid}', uid)
        return jsonify({'ok': True})
    except Exception as e:
        conn.rollback(); return jsonify({'ok': False, 'error': str(e)}), 400
    finally: cur.close(); conn.close()
...
def crear_modulo():
    ...
    try:
        cur.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s)
        """, (d['clave'].strip().lower(), d['nombre'].strip(),
              d.get('descripcion','').strip(), d.get('icono','📦'),
              int(d.get('orden', 99))))
        conn.commit()
        return jsonify({'ok': True, 'id': cur.lastrowid})
    except Exception as e:
...
def eliminar_correo(id):
    ...
    conn = get_connection(); cur = conn.cursor()
    try:
        cur.execute("DELETE FROM config_correo WHERE id=%s AND usuario_id=%s", (id, usuario_id))
        conn.commit()
        return jsonify({'ok': True})
    except Exception as e:
        conn.rollback()
...
def probar_correo():
    ...
    resultado = enviar_correo(destinatario, 'Prueba - Sistema de Convalidaciones', cuerpo_html)
