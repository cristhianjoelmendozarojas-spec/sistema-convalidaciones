...
class FullPageImage(Flowable):
    ...
    def draw(self):
        self.canv.drawImage(
            self.path, 0, 0, width=PAGE_W, height=PAGE_H, preserveAspectRatio=False
        )
...
def build_pdf(solicitud_id, datos):
    ...
    story.append(
        Paragraph(
            '"Año de la Esperanza y el Fortalecimiento de la Democracia"', S["decreto"]
        )
    )
    ...
    story.append(
        Paragraph(
            f"{datos.get('fecha', '')} de {datos.get('mes', '')} del {datos.get('anio', '')}",
            S["fecha"],
        )
    )

    story.append(
        Paragraph(f"<b><u>{datos.get('codigo', '')}</u></b>", S["dest_codigo"])
    )
    ...
    asunto_data = [
        [
            ...
            Paragraph(": Simulación de convalidación", S["asunto_texto"]),
        ]
    ]
    asunto_table = Table(asunto_data, colWidths=[3.0 * cm, CONTENT_W - 3.0 * cm])
    asunto_table.setStyle(
        TableStyle(
            [
                ("VALIGN", (0, 0), (-1, -1), "TOP"),
    ...
    story.append(
        Paragraph(
            f"Reciba usted un cordial saludo a nombre {facultad} de la Universidad Autónoma de Ica.",
            S["body"],
        )
    )
    ...
    story.append(
        Paragraph(
            "Dicha propuesta está sujeta a términos y condiciones académicas y administrativas contenidas en el <b>Anexo 02</b>, que están asociadas a nuestro servicio educativo, por lo que adjuntamos la información detalladamente para que usted como persona interesada pueda revisarla.",
            S["body"],
        )
    )
    story.append(
        Paragraph(
            "Asimismo, en la simulación se podrá verificar las asignaturas por convalidación directa y la cantidad de exámenes propuestos, mismos que deberán ser aplicados y aprobados para que se pueda dar por convalidado.",
            S["body"],
        )
    )
    story.append(
        Paragraph(
            "Es importante mencionar que para seguir con el procedimiento de convalidación la persona interesada debe confirmar la aceptación del presente documento a través del correo de <u><a href='mailto:admision.extraordinaria@autonomadeica.edu.pe' color='#0070C0'>admision.extraordinaria@autonomadeica.edu.pe</a></u> para lo cual tiene un plazo de 48 horas después de notificada dicha respuesta.",
            S["body"],
        )
    )
    ...
    story.append(
        Paragraph(
            f'En la simulación se podrá verificar las asignaturas por convalidación directa al programa de estudios de {programa}, para {tratamiento} proveniente del <b>"{ies_origen}"</b>, así como el número de asignaturas no convalidadas y exámenes de suficiencia propuestos por la Facultad.',
            S["body"],
        )
    )
    story.append(Spacer(1, 0.3 * cm))
    ...
    story.append(
        Paragraph(
            "Cuadro N°01. Asignaturas y créditos convalidados", S["titulo_cuadro"]
        )
    )
    ...
    for r in cursos_conv:
        c01_data.append(
            [
                Paragraph(str(r.get("ciclo", "")), S["tabla_cell"]),
                Paragraph(r.get("nombre", ""), S["tabla_cell_left"]),
                Paragraph(str(r.get("creditos", "")), S["tabla_cell"]),
            ]
        )
    c01_data.append(
        [
            Paragraph("", S["tabla_footer"]),
            Paragraph("TOTAL DE CRÉDITOS CONVALIDADOS", S["tabla_footer"]),
            Paragraph(str(total_conv), S["tabla_footer"]),
        ]
    )

    c01 = Table(
        c01_data, colWidths=[2.0 * cm, CONTENT_W - 5.0 * cm, 3.0 * cm], repeatRows=1
    )
    ...
    story.append(
        Paragraph("Cuadro N°02. Costo de créditos convalidados", S["titulo_cuadro"])
    )
    ...
    c02_data = [
        ...
        [
            ...
            Paragraph(f"S/ {costo_credito:.2f}", S["tabla_cell"]),
    ...
    story.append(
        Paragraph(
            "Cuadro N°03. Asignaturas y créditos por convalidar mediante examen de suficiencia",
            S["titulo_cuadro"],
        )
    )
    ...
    for r in cursos_suf:
        c03_data.append(
            [
                ...
                Paragraph(str(r.get("creditos", "")), S["tabla_cell"]),
                Paragraph(
                    r.get("denominacion", "EXAMEN DE SUFICIENCIA"), S["tabla_cell"]
                ),
            ]
        )
    c03_data.append(
        [
            Paragraph("", S["tabla_footer"]),
            Paragraph("TOTAL DE CRÉDITOS", S["tabla_footer"]),
            Paragraph(str(total_suf), S["tabla_footer"]),
            Paragraph("", S["tabla_footer"]),
        ]
    )
    ...
    story.append(
        Paragraph("Cuadro N°04. Costo del examen de suficiencia", S["titulo_cuadro"])
    )
    ...
    num_examenes = len(
        [
            c
            for c in cursos_suf
            if c.get("denominacion", "").upper().startswith("EXAMEN")
        ]
    )
    ...
    c04_data = [
        ...
        [
            ...
            Paragraph(f"S/ {costo_examen:.2f}", S["tabla_cell"]),
    ...
    story.append(
        Paragraph("Cuadro N°05. Costo de convalidación total", S["titulo_cuadro"])
    )
    ...
    c05_data = [
        ...
        [
            ...
            Paragraph(f"S/ {total_general:.2f}", S["tabla_footer"]),
        ],
    ]
    ...
    story.append(
        Paragraph("Cuadro N°06. Asignaturas no convalidadas", S["titulo_cuadro"])
    )
    ...
    for r in cursos_no:
        c06_data.append(
            [
                ...
                Paragraph(r.get("nombre", ""), S["tabla_cell_left"]),
                Paragraph(str(r.get("creditos", "")), S["tabla_cell"]),
                Paragraph(r.get("periodo", ""), S["tabla_cell"]),
            ]
        )
    c06_data.append(
        [
    ...
    terminos = [
        (
            ...
            "Denominamos usuario del servicio educativo a la persona que decide voluntariamente postular, matricularse y/o convalidar en la Universidad Autónoma de Ica, en adelante, UAI y que hace efectivo dicho proceso administrativamente cancelando los derechos establecidos y presentando la ficha de inscripción y los requisitos correspondientes.",
        ),
        (
            "2.",
            "Voluntariedad de los procesos:",
            "La postulación mediante el proceso de admisión, la matrícula y la solicitud de evaluación por convalidación respectivamente, son actos voluntarios, por lo que, al ejecutar su decisión, el usuario ha recibido información, ha consultado y tiene clara la información proporcionada y vinculada nuestro servicio a nivel académico y administrativo. Así mismo, después de realizado los pagos no se harán devoluciones.",
        ),
        (
            "3.",
            "Documentación Administrativa:",
            "Toda documentación presentada como parte del proceso de admisión, matrícula o convalidación formará parte del expediente académico del usuario y no será devuelta una vez iniciado el trámite.",
        ),
    ]
    ...
    doc = SimpleDocTemplate(
        ...
        title="Simulación de Convalidación",
