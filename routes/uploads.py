...
def get_checklist(postulante_id):
    ...
    for doc in docs:
        if doc.get("documento"):
            ...
            doc["es_imagen"] = doc["documento"].lower().split(".")[-1] in {
                "png",
                "jpg",
                "jpeg",
                "gif",
...
def upload_foto_postulante(postulante_id):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "UPDATE postulantes SET foto = %s WHERE id = %s", (filename, postulante_id)
        )
        conn.commit()
        cur.close()
        conn.close()
        ...
        return jsonify(
            {"ok": True, "filename": filename, "url": f"/uploads/{filename}"}
        )
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
