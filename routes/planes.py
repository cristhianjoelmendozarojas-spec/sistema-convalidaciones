...
def _validar_fila(fila_dict):
    ...
    try:
        ...
        if c is None or c <= 0: errores.append('Créditos debe ser entero positivo')
    except: errores.append('Créditos no es número')
...
def index():
    ...
    return render_template('planes/lista.html',
        locales=locales, externos=externos, nombres_existentes=nombres_existentes)
...
def importar():
    ...
    try:
        ...
        for col in ['CICLO','CÓDIGO','NOMBRE DEL CURSO','CREDITOS','PRERREQUISITO']:
            try: col_map[col] = encabezados.index(col)
        ...
        for num, fila in enumerate(datos, 2):
            fila_dict = {col: (fila[idx] if idx < len(fila) else None) for col, idx in col_map.items()}
            es_valida, es_total, errs = _validar_fila(fila_dict)
            if es_total: continue
            if es_valida: filas_validas.append(fila_dict)
            else: errores_total.append(f'Fila {num}: {", ".join(errs)}')
...
def descargar_plantilla():
    ...
    wb = openpyxl.Workbook(); ws = wb.active; ws.title = 'Plan de Estudios'
    borde = Border(left=Side(style='thin',color='CCCCCC'), right=Side(style='thin',color='CCCCCC'),
                   top=Side(style='thin',color='CCCCCC'),  bottom=Side(style='thin',color='CCCCCC'))
    for i, (t, w) in enumerate(zip(['CICLO','CÓDIGO','NOMBRE DEL CURSO','CREDITOS','PRERREQUISITO'],[8,14,40,12,14]),1):
        c = ws.cell(row=1,column=i,value=t)
        c.font=Font(bold=True,color='FFFFFF',size=10); c.fill=PatternFill('solid',fgColor='0C1D3A')
        c.alignment=Alignment(horizontal='center',vertical='center'); c.border=borde
        ws.column_dimensions[c.column_letter].width=w
    ws.row_dimensions[1].height=28
    for ri,fila in enumerate([('I','P02A1101','Matemática I',4,''),('I','P02A1102','Redacción y Comunicación',4,''),
                               ('II','P02A1107','Matemática II',4,'P02A1101'),('III','','Electivo 1',2,'')],2):
        for ci,v in enumerate(fila,1):
            c=ws.cell(row=ri,column=ci,value=v)
            c.font=Font(color='888888',italic=True,size=9); c.border=borde
            c.alignment=Alignment(horizontal='center' if ci in (1,4) else 'left',vertical='center')
    ws.freeze_panes='A2'
    buf=io.BytesIO(); wb.save(buf); buf.seek(0)
    return send_file(buf,as_attachment=True,download_name='plantilla_plan_estudios.xlsx',
...
def cursos(plan_id):
    ...
    plan = cur.fetchone()
    if not plan: return jsonify({'ok': False}), 404
    ...
    cursos = cur.fetchall()
    cur.close(); conn.close()
    return jsonify({'ok': True, 'plan': plan['nombre_plan'],
                    'periodo': plan['periodo_academico'], 'cursos': cursos})
...
def editar(plan_id):
    ...
    conn = get_connection(); cur = conn.cursor()
    try:
        cur.execute("""UPDATE planes_estudio
                       SET nombre_plan=%s, tipo_plan=%s, periodo_academico=%s
                       WHERE id=%s""",
                    (d.get('nombre_plan','').strip(), d.get('tipo_plan','local'),
                     d.get('periodo_academico','').strip(), plan_id))
