...
from routes.postulantes import (
    _limpiar_fecha, _limpiar_monto, _norm_texto, _norm_valor,
    _leer_archivo, COLUMNAS, CAMPOS_PROTEGIDOS, CAMPO_LABELS,
    IDX_ESTADO_EXPEDIENTE, IDX_ESTADO_POSTULACION, IDX_ESTADO_MATRICULA,
    _CAMPOS_MONTO, _CAMPOS_FECHA, _CAMPOS_INT, _CAMPOS_UPPER,
)
...
test('_limpiar_monto(S/.150.00) -> se pierde',
...
VACIOS = {'', 'None', 'nan', 'NaT', 'none', 'null', '0000-00-00', '0000-00-00 00:00:00', 'NULL'}
# Para montos, _norm_valor llama a _limpiar_monto, que con S/. falla
test('_norm_valor(monto, 150.00)', _norm_valor('monto_expediente', '150.00', VACIOS) == '150.00')
test('_norm_valor(fecha, 15/03/2026)', _norm_valor('fecha_nacimiento', '15/03/2026', VACIOS) == '2026-03-15')
test('_norm_valor(edad, 24)', _norm_valor('edad', '24', VACIOS) == '24')
test('_norm_valor(texto upper)', _norm_valor('apellidos_nombres', '  juan  perez  ', VACIOS) == 'JUAN PEREZ')
test('_norm_valor(texto generico)', _norm_valor('correo', '  TEST@example.COM  ', VACIOS) == 'TEST@example.COM')
...
DATOS_PRUEBA = [
    ...
    ['DNI', '12345678', 'EST-001', 'GARCIA LOPEZ JUAN CARLOS',
     '987654321', 'juan@example.com', 'ICA', 'ICA', 'CHINCHA',
     'M', '15/03/2000', 26, 'CHINCHA', 'CIENCIAS DE LA SALUD',
     'ENFERMERÍA', 'Graduados y Titulados', '2026-I',
     'Presencial', 'Noche', 'MARIA GONZALES', '10/01/2026',
     'A', 'A', 150.00, 'PAGADO', '15/01/2026',
     100.00, 'PAGADO', '20/01/2026',
     200.00, 'PENDIENTE', ''],
    # 1: Postulante con campos vacios
    ['DNI', '87654321', 'EST-002', 'QUISPE MAMANI MARIA',
     '', 'maria@example.com', 'LIMA', 'LIMA', 'MIRAFLORES',
     'F', '20/05/1999', 27, 'LIMA', 'INGENIERÍA',
     'SISTEMAS', 'Ingreso Directo', '2026-I',
     'Presencial', 'Tarde', 'PEDRO RAMIREZ', '05/01/2026',
     'B', 'B', '', '', '',
     '', '', '',
     '', '', ''],
    # 2: Postulante con valores abiertos en Excel (None)
    [None, '99887766', 'EST-003', None,
    ...
    ['DNI', '55556666', '', 'INVALIDO SIN CODIGO',
     '111222333', '', '', '', '',
     '', '', '', '', '',
     '', '', '',
     '', '', '', '',
    ...
    ['DNI', '44443333', 'EST-004', 'DIAZ GARCIA CARLA',
     '999000111', 'carla@mail.com', 'CUSCO', 'CUSCO', 'WANCHAQ',
     'F', datetime(2002, 7, 15), 23, 'CUSCO', 'CONTABILIDAD',
     'CONTABILIDAD', 'Ordinario', '2026-I',
     'Presencial', 'Mañana', 'LUZ MENDOZA', datetime(2026, 3, 1),
     'A', 'A', 180.00, 'PAGADO', datetime(2026, 3, 5),
     90.00, 'PENDIENTE', datetime(2026, 3, 5),
     250.00, 'PENDIENTE', datetime(2026, 3, 5)],
]
...
def generar_excel_orden_invalido(nombre):
    ...
    ws.append(['EST-010', 'APELLIDOS TEST', '87654321', 'DNI',
               '999999999', 'test@test.com', 'LIMA', 'LIMA', 'LIMA', 'M',
               '01/01/2000', 26, 'LIMA', 'TEST', 'TEST', 'Ordinario', '2026-I',
               'Presencial', 'Mañana', 'ASESORA', '01/01/2026',
               'A', 'A', 999.00, 'PAGADO', '01/01/2026',
               888.00, 'PAGADO', '01/01/2026',
               777.00, 'PENDIENTE', '01/01/2026'])
    wb.save(path)
    return path
...
ARCHIVOS['xlsx_orden_invalido'] = generar_excel_orden_invalido('test_orden_invalido.xlsx')
# Solo encabezados sin datos
ARCHIVOS['xlsx_vacio'] = generar_excel([['DNI', ' ', '', '', '', '', '', '', '', '', '', '', '', '', '',
                                          '', '', '', '', '', '', '', '', '', '', '', '', '', '',
                                          '', '', '']][:0], 'test_vacio.xlsx')
...
class FlaskFile:
    ...
    def read(self, *a, **kw): return self._fp.read(*a, **kw)
    def seek(self, *a, **kw): return self._fp.seek(*a, **kw)
    def tell(self, *a, **kw): return self._fp.tell(*a, **kw)
    def seekable(self): return True
    def close(self): self._fp.close()
    @property
    def filename(self): return os.path.basename(self._path)
...
for nombre, ruta in ARCHIVOS.items():
    ...
    try:
        ...
        if nombre == 'xlsx_vacio':
            test(f'_leer_archivo({nombre}) -> headers OK, 0 filas',
                 enc is not None and cant == 0)
        else:
            test(f'_leer_archivo({nombre}) -> {cant} filas',
                 enc is not None and cant > 0,
                 f'encabezados={enc[:3]}...')
...
enc, filas = _leer_archivo(ff, '.xlsx'); ff.close()
...
test('numero cols mapeadas', len(col_map) == 29,
     f'esperado=29, obtenido={len(col_map)}')
test('estado* NO en col_map (por posicion)',
     'estado_expediente' not in col_map)
...
def simular_parseo(fila, col_map, fila_raw):
    fila_dict = {campo: (fila[idx] if idx < len(fila) else None)
                 for campo, idx in col_map.items() if campo not in ('estado_expediente','estado_postulacion','estado_matricula')}
...
for fila in filas:
    ...
    if not codigo:
        ...
        continue
    nuevos += 1
    test(f'{codigo}: DNI={fila_dict.get("dni")}',
         str(fila_dict.get('dni','')) not in ('', 'None'))

test('Nuevos postulantes parseados', nuevos == 3,
...
_, filas2 = _leer_archivo(ff2, '.xlsx'); ff2.close()
for f in filas2:
    codigo, fd = simular_parseo(f, col_map, f)
    if codigo == 'EST-001':
        test('EST-001: DNI', str(fd['dni']) == '12345678')
        test('EST-001: monto_exp=150.00', str(fd['monto_expediente']) == '150.0' or str(fd['monto_expediente']) == '150')
...
for f in filas2:
    ...
    if codigo == 'EST-004':
        ...
        test('EST-004: f_nacimiento como datetime', isinstance(fd['fecha_nacimiento'], datetime))
        test('EST-004: f_registro como datetime', isinstance(fd['fecha_registro_origen'], datetime))
        break
...
for f in filas2:
    codigo, fd = simular_parseo(f, col_map, f)
    if codigo == 'EST-003':
        test('EST-003: todos None', all(v is None for v in fd.values()),
             f'Valores: {dict(fd)}')
        break
...
ff = FlaskFile(ARCHIVOS['xlsx_normal'])
_, filas_r = _leer_archivo(ff, '.xlsx'); ff.close()
col_map_est = {}
for nombre_col, campo_bd in COLUMNAS.items():
    try: col_map_est[campo_bd] = enc.index(nombre_col)
...
enc_inv, filas_inv = _leer_archivo(ff, '.xlsx'); ff.close()

col_map_inv = {}
for nombre_col, campo_bd in COLUMNAS.items():
    try: col_map_inv[campo_bd] = enc_inv.index(nombre_col)
    except ValueError: pass
...
print(f'  Columna en indice {IDX_ESTADO_EXPEDIENTE}: {enc_inv[IDX_ESTADO_EXPEDIENTE]!r} -> valor={ee!r}')
print(f'  Columna en indice {IDX_ESTADO_POSTULACION}: {enc_inv[IDX_ESTADO_POSTULACION]!r} -> valor={ep!r}')
print(f'  Columna en indice {IDX_ESTADO_MATRICULA}: {enc_inv[IDX_ESTADO_MATRICULA]!r} -> valor={em!r}')

# Como reordenamos columnas, los indices fijos leen columnas equivocadas
# Verificar si al menos leyo algo distinto de vacio
test('  Estados leidos (pueden ser incorrectos por orden cambiado)',
     ee != '' or ep != '' or em != '',
...
enc_csv, filas_csv = _leer_archivo(ff, '.csv'); ff.close()
test('CSV: headers OK', enc_csv is not None)
test(f'CSV: {len(filas_csv)} filas', len(filas_csv) == 5)

col_map_csv = {}
for nombre_col, campo_bd in COLUMNAS.items():
    try: col_map_csv[campo_bd] = enc_csv.index(nombre_col)
    except ValueError: pass
...
ff = FlaskFile(p)
enc_v, filas_v = _leer_archivo(ff, '.xlsx'); ff.close()
...
test('norm_bd(monto, 150.00)',
     _normalizar_para_bd('monto_expediente', '150.00') == 150.00)
test('norm_bd(monto, "0")',
     _normalizar_para_bd('monto_expediente', '0') == 0.0)
test('norm_bd(fecha, dd/mm/yyyy)',
     _normalizar_para_bd('fecha_nacimiento', '15/03/2026') == '2026-03-15')
test('norm_bd(edad, 26)',
     _normalizar_para_bd('edad', '26') == 26)
test('norm_bd(edad, "0")',
     _normalizar_para_bd('edad', '0') == 0)
test('norm_bd(texto, JUAN)',
     _normalizar_para_bd('apellidos_nombres', 'JUAN') == 'JUAN')
test('norm_bd(texto vacio)',
     _normalizar_para_bd('apellidos_nombres', '') is None)
test('norm_bd(None)',
     _normalizar_para_bd('turno', None) is None)
...
for nombre, ruta in ARCHIVOS.items():
    ...
    if 'error' in res:
        ...
        test(f'Import {nombre}: {res["nuevos"]} nuevos, {res.get("errores",0)} errores',
             res['errores'] == 0, f'res={res}')
