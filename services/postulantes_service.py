...
def get_postulantes_lista(page=1, per_page=20, filtros=None):
    ...
    total = fetch_one(f"SELECT COUNT(*) as total FROM postulantes WHERE {where_sql}", tuple(params))
...
def crear_postulante(data):
    ...
    with Database(dictionary=False) as db:
        ...
        """, (
            data.get('codigo'), data.get('tipo_documento'), data.get('dni'),
            data.get('apellidos_nombres'), data.get('celular'), data.get('correo'),
            data.get('departamento'), data.get('provincia'), data.get('distrito'),
            data.get('sexo'), data.get('fecha_nacimiento'), data.get('edad'),
            data.get('local'), data.get('facultad'), data.get('programa'),
            data.get('modalidad_admision'), data.get('semestre_academico'),
            data.get('modalidad_estudios'), data.get('turno'), data.get('asesora')
        ))
        db.commit()
...
def actualizar_postulante(postulante_id, data):
    ...
    for campo in ['tipo_documento', 'dni', 'apellidos_nombres', 'celular', 'correo',
                'departamento', 'provincia', 'distrito', 'sexo', 'fecha_nacimiento', 'edad',
                'local', 'facultad', 'programa', 'modalidad_admision', 'semestre_academico',
                'modalidad_estudios', 'turno', 'asesora']:
        if campo in data:
...
def get_estadisticas_postulantes():
    ...
    r = fetch_one("SELECT COUNT(*) as total FROM postulantes WHERE EXTRACT(YEAR FROM fecha_importacion)=EXTRACT(YEAR FROM NOW())")
